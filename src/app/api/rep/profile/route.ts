...
const MONTHLY_FLAT = 25;    // Pro plan $/month
const PER_REWARD = 1.25;    // $/reward redemption
...
export async function GET() {
  try {
    ...
    // Get this rep's shops
    const { data: shops } = await admin
      .from("shops")
      .select("slug, plan_type, subscription_status, rep_claimed_at")
      .eq("rep_id", profile.id);

    const myShops = shops ?? [];
    const activePro = myShops.filter(s => s.plan_type === "pro" && s.subscription_status === "active");
    const activeFree = myShops.filter(s => s.plan_type !== "pro");
    ...
    if (slugs.length > 0) {
      ...
      rewardCommission = (count ?? 0) * PER_REWARD * COMMISSION_RATE;
    }

    // All-time commission estimate (pro months since claim)
    let allTimeCommission = 0;
    for (const shop of activePro) {
      const claimedAt = shop.rep_claimed_at ? new Date(shop.rep_claimed_at) : new Date();
